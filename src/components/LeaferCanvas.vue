...
import { App, Rect, Text, Group, Box, PointerEvent, DragEvent, Ellipse, Line, Path } from 'leafer-ui'
...
const lineMap = new Map<string, Array<{ line: Path, arrow: Path, type: 'incoming' | 'outgoing', otherId: string }>>()
const taskPositions = new Map<string, { x: number, y: number, width: number, height: number, phaseId: string }>()
    ...
    containerRef.value.addEventListener('wheel', (e: WheelEvent) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        if (e.shiftKey && e.deltaX === 0) {
         if (leaferApp && leaferApp.tree) leaferApp.tree.x = (leaferApp.tree.x || 0) - e.deltaY
      } else {
...
const updateHandleGroup = () => {
    ...
    const dep = task.dependencies.find(d => (typeof d === 'string' ? d : d.taskId) === depId)
    // if (!dep) return // Might be string
    ...
    const forcedSource = typeof dep === 'object' ? dep.sourcePort : undefined
    const forcedTarget = typeof dep === 'object' ? dep.targetPort : undefined
    
    const points = getBestConnectionPoints(sourcePos, targetPos, forcedSource, forcedTarget, sourceTask, task)
    ...
    if (depType === 'curve') {
        ...
        const cps = getCurveControlPoints(points.startX, points.startY, points.endX, points.endY, points.startSide, points.endSide, depObj?.controlPoints, depObj?.controlPointCount)
        ...
        const helperLine1 = new Line({ points: [], stroke: '#E91E63', strokeWidth: 1, dashPattern: [3, 3] })
        const helperLine2 = new Line({ points: [], stroke: '#E91E63', strokeWidth: 1, dashPattern: [3, 3] })
        const helperLine3 = new Line({ points: [], stroke: '#E91E63', strokeWidth: 1, dashPattern: [3, 3] }) // For mid section if needed
        ...
        if (cps.length === 2) {
            ...
            handleGroup.add(helperLine2)
             ...
             const cp1 = cps[1]!
             const cp2 = cps[2]!
             helperLine1.points = [points.startX, points.startY, cp0.x, cp0.y, cp1.x, cp1.y]
             helperLine2.points = [cp1.x, cp1.y, cp2.x, cp2.y, points.endX, points.endY]
             handleGroup.add(helperLine1)
             handleGroup.add(helperLine2)
        ...
        const setupCPDrag = (handle: Ellipse, index: number) => {
            ...
            handle.on(DragEvent.DRAG, (e: DragEvent) => {
                ...
                if (cps.length === 2) {
                     if (index === 0) {
                         helperLine1.points = [points.startX, points.startY, (handle.x || 0) + hRadius, (handle.y || 0) + hRadius]
                     } else {
                         helperLine2.points = [points.endX, points.endY, (handle.x || 0) + hRadius, (handle.y || 0) + hRadius]
                     }
                } else if (cps.length === 3) {
                     ...
                     if (index === 0) {
                         ...
                         if (pts.length >= 6) { // [sx, sy, c1x, c1y, c2x, c2y]
                         ...
                         if (pts2.length >= 6) { // [c2x, c2y, c3x, c3y, ex, ey]
                    ...
                    if (index === 0) {
                        ...
                        // CP4 -> Update Helper 3
                        helperLine3.points = [(handle.x || 0) + hRadius, (handle.y || 0) + hRadius, points.endX, points.endY]
                    }
                }
                ...
                if (lineObj && lineObj.line) {
                    ...
                    const currentCPs = handles.map(h => ({ x: (h.x || 0) + r, y: (h.y || 0) + r }))
                    
                    const pathData = getCurvePathData(points.startX, points.startY, points.endX, points.endY, currentCPs)
                    lineObj.line.path = pathData
                    
                    // Update Arrow Rotation
                    const lastCP = currentCPs[currentCPs.length - 1]
                    const angle = lastCP ? Math.atan2(points.endY - lastCP.y, points.endX - lastCP.x) * 180 / Math.PI : 0
                    if (lineObj.arrow) {
            ...
            handle.on(DragEvent.END, () => {
                ...
                const offsets = handles.map((h, i) => {
                    ...
                    if (handles.length === 2) {
                        ...
                        if (i === 0) return { x: hx - points.startX, y: hy - points.startY }
    ...
    const setupDrag = (handle: Ellipse, type: 'source' | 'target') => {
        ...
        handle.on(DragEvent.START, () => {
            ...
            isDragging = true
                ghost = new Ellipse({ 
                    x: handle.x || 0, y: handle.y || 0, width: handle.width, height: handle.height,
                    fill: 'transparent', stroke: '#f00', strokeWidth: 2 
                })
        ...
        handle.on(DragEvent.DRAG, (e: DragEvent) => {
            ...
            for (const [tid, pos] of taskPositions.entries()) {
                if (gx >= pos.x && gx <= pos.x + pos.width &&
                    gy >= pos.y && gy <= pos.y + pos.height) {
                    hoveredTask = tid
                    ...
                    feedbackGroup?.add(new Rect({
                        x: pos.x, y: pos.y, width: pos.width, height: pos.height,
                        stroke: '#2196F3', strokeWidth: 2, fill: 'transparent'
                    }))
        ...
        handle.on(DragEvent.END, () => {
            ...
            if (ghost) {
                 ...
                 const targetTaskPos = droppedTaskPos || (type === 'source' ? sourcePos : targetPos)
                 const targetIdToCheck = droppedTaskId || (type === 'source' ? depId : taskId)
                 const targetTaskToCheck = store.tasks.find(t => t.id === targetIdToCheck)
                ...
                if (closest) {
                    ...
                    if (type === 'source') {
                        ...
                        if (droppedTaskId && droppedTaskId !== depId) {
                            ...
                            store.changeDependencySource(taskId, depId, droppedTaskId, closest as any)
                        ...
                        if (droppedTaskId && droppedTaskId !== taskId) {
                             ...
                             store.moveDependencyTarget(taskId, droppedTaskId, depId, closest as any)
                             store.clearSelection()
                        } else {
                            // Same task, just updating port
                            store.updateDependencyPort(taskId, depId, type, closest as any)
...
const drawChart = () => {
  ...
  const visibleTasks = store.tasks.filter(t => {
      if (statuses && statuses.length && !statuses.includes(t.status)) return false
      if (phases && phases.length && !phases.includes(t.phaseId)) return false
      if (swimlanes && swimlanes.length && !swimlanes.includes(t.swimlaneId)) return false
      return true
  })
  
  const isFiltering = (statuses && statuses.length) || (phases && phases.length) || (swimlanes && swimlanes.length)
  ...
  if (isFiltering) {
      ...
      visibleSwimlanes = store.swimlanes.filter(s => activeSwimlaneIds.has(s.id))
  }
  ...
  lastContentHeight = HEADER_HEIGHT + swimlaneHeights.reduce((s, h) => s + h, 0)
  ...
  visiblePhases.forEach((phase, index) => {
    ...
    const isSelected = store.selectedElement?.type === 'phase' && store.selectedElement.id === phase.id
  ...
  visibleSwimlanes.forEach((lane, laneIndex) => {
    ...
    const isSelected = store.selectedElement?.type === 'swimlane' && store.selectedElement.id === lane.id
    ...
    visiblePhases.forEach((phase, index) => {
      ...
      const tasksInCell = visibleTasks.filter(t => 
        t.phaseId === phase.id && 
        t.swimlaneId === lane.id
      )
      ...
      tasksInCell.forEach(task => {
        ...
        if (task.x !== undefined && task.y !== undefined) {
             ...
             while (!placed && attempts < 100) {
                 ...
                 const currentRect = { x: flowX, y: flowY, width: TASK_WIDTH, height: TASK_HEIGHT }
                 let hasCollision = false
                 
                 for (const rect of occupiedRects) {
                     // Simple AABB collision
                     if (currentRect.x < rect.x + rect.width &&
             ...
             occupiedRects.push({ x: finalX, y: finalY, width: TASK_WIDTH, height: TASK_HEIGHT })
  ...
  visibleTasks.forEach(task => {
    ...
    task.dependencies.forEach(dep => {
      ...
      const forcedSourceSide = typeof dep === 'object' ? dep.sourcePort : undefined
      const forcedTargetSide = typeof dep === 'object' ? dep.targetPort : undefined
      ...
      // Use helper to find best connection points
       const bestPoints = getBestConnectionPoints(startPos, endPos, forcedSourceSide, forcedTargetSide, startTask, task)
       ...
       if (depType === 'straight') {
           pathData = `M ${startX} ${startY} L ${endX} ${endY}`
             ...
             const depObj = (typeof dep === 'object' ? dep : null)
             const cps = getCurveControlPoints(startX, startY, endX, endY, startSide, endSide, depObj?.controlPoints, depObj?.controlPointCount)
             pathData = getCurvePathData(startX, startY, endX, endY, cps)
        } else {
           pathData = `M ${startX} ${startY} L ${exitX} ${exitY} ${route} L ${endX} ${endY}`
       }
      
      const isInput = store.selectedElement?.type === 'task' && store.selectedElement.id === task.id
      const isOutput = store.selectedElement?.type === 'task' && store.selectedElement.id === depId
      
      const isSelectedDep = store.selectedElement?.type === 'dependency' && 
      ...
      const line = new Path({
        ...
        zIndex: (isInput || isOutput || isSelectedDep) ? 100 : (isAnimating ? 50 : 0),
        cursor: 'pointer'
      })
      ...
      line.data = { type: 'dependency', id: depKey, sourceId: depId, targetId: task.id }
      ...
      if (depType === 'straight') {
           ...
           angle = lastCP ? Math.atan2(endY - lastCP.y, endX - lastCP.x) * 180 / Math.PI : 0
      } else {
          if (endSide === 'left') {
      ...
      lineMap.get(task.id)!.push({ line, arrow, type: 'incoming', otherId: depId })
      lineMap.get(depId)!.push({ line, arrow, type: 'outgoing', otherId: task.id })
    })
  })
...
const startConnection = (sourceId: string, startX: number, startY: number, side: string) => {
    ...
    tempArrow.path = `M ${startX} ${startY} L ${startX-12} ${startY-6} L ${startX-12} ${startY+6} Z`
...
const getCurveControlPoints = (
    ...
    startSide: string, endSide: string,
    ...
    if (existingPoints && (!requestedCount || existingPoints.length === requestedCount)) {
    ...
    if (requestedCount === 2 || (!requestedCount && (!existingPoints || existingPoints.length !== 4))) {
    ...
    if (existingPoints && existingPoints.length === 3) {
         ...
         return [
             { x: c1x, y: c1y },
             { x: (c1x + c3x) * 0.33 + c2x * 0.33, y: (c1y + c3y) * 0.33 + c2y * 0.33 }, // Rough
             { x: (c1x + c3x) * 0.66 + c2x * 0.33, y: (c1y + c3y) * 0.66 + c2y * 0.33 },
             { x: c3x, y: c3y }
         ]
    }
...
const getCurvePathData = (startX: number, startY: number, endX: number, endY: number, cps: {x:number, y:number}[]) => {
    if (cps.length === 4) {
        const cp0 = cps[0]!
        const cp1 = cps[1]!
        const cp2 = cps[2]!
...
const drawTaskNode = (group: Group, task: any, x: number, y: number) => {
  const isTaskSelected = store.selectedElement?.type === 'task' && store.selectedElement.id === task.id
  const isAnyPortSelected = store.selectedElement?.type === 'port' && store.selectedElement.taskId === task.id
  ...
  const defaultPortsConfig = [
      ...
      { id: 'right', x: TASK_WIDTH - 6, y: TASK_HEIGHT / 2 - 6, stroke: '#2196F3' },
      { id: 'top', x: TASK_WIDTH / 2 - 6, y: -6, stroke: '#2196F3' },
      { id: 'bottom', x: TASK_WIDTH / 2 - 6, y: TASK_HEIGHT - 6, stroke: '#2196F3' }
  ]

  defaultPortsConfig.forEach(cfg => {
      const port = new Ellipse({
          x: cfg.x, y: cfg.y, width: 12, height: 12,
          fill: '#fff', stroke: cfg.stroke, strokeWidth: 2,
          // Fixed ports are not draggable for position adjustment, but draggable for connection
          cursor: 'crosshair', visible: showPorts // Show if selected
      })
      port.data = { type: 'port', portType: cfg.id, taskId: task.id, isCustom: false }
      allPorts.push(port)
      nodeGroup.add(port)
  })
  ...
  if (task.ports) {
      task.ports.forEach((p: any) => {
          let px = 0, py = 0
          if (p.side === 'top') { px = TASK_WIDTH * p.percentage; py = 0 }
          else if (p.side === 'bottom') { px = TASK_WIDTH * p.percentage; py = TASK_HEIGHT }
          else if (p.side === 'left') { px = 0; py = TASK_HEIGHT * p.percentage }
          else if (p.side === 'right') { px = TASK_WIDTH; py = TASK_HEIGHT * p.percentage }
          
          const isPortSelected = store.selectedElement?.type === 'port' && store.selectedElement.id === p.id
          ...
          const port = new Ellipse({
              ...
              cursor: 'crosshair', visible: showPorts
          })
          port.data = { type: 'port', portType: p.id, taskId: task.id, isCustom: true, side: p.side, percentage: p.percentage }
  ...
  const disableDrag = () => { nodeGroup.draggable = false }
  const enableDrag = () => { 
    if (!isConnecting.value) {
  ...
  nodeGroup.on(PointerEvent.MOVE, (e: PointerEvent) => {
      if (e.ctrlKey) {
          ...
          const threshold = 15
          let nearEdge = false
          if (Math.abs(relY) < threshold || Math.abs(relY - TASK_HEIGHT) < threshold || 
              Math.abs(relX) < threshold || Math.abs(relX - TASK_WIDTH) < threshold) {
              nearEdge = true
          }
  ...
  allPorts.forEach(port => {
    port.on(PointerEvent.ENTER, (e: PointerEvent) => {
        disableDrag()
        const isSelected = store.selectedElement?.type === 'port' && store.selectedElement.id === port.data.portType
        ...
        if (isSelected && port.data.isCustom) {
            ...
            const onUp = () => {
                if (port.data.newPercentage !== undefined) {
                    store.updateTaskPort(task.id, port.data.portType, port.data.newPercentage)
...
const getAllTaskPorts = (
    ...
    const ports = [
        ...
        { id: 'bottom', side: 'bottom', x: pos.x + pos.width/2, y: pos.y + pos.height },
        { id: 'left', side: 'left', x: pos.x, y: pos.y + pos.height/2 },
        { id: 'right', side: 'right', x: pos.x + pos.width, y: pos.y + pos.height/2 }
    ]
    ...
    if (task && task.ports) {
        task.ports.forEach(p => {
            let px = 0, py = 0
            if (p.side === 'top') { px = pos.x + pos.width * p.percentage; py = pos.y }
            else if (p.side === 'bottom') { px = pos.x + pos.width * p.percentage; py = pos.y + pos.height }
            else if (p.side === 'left') { px = pos.x; py = pos.y + pos.height * p.percentage }
            else if (p.side === 'right') { px = pos.x + pos.width; py = pos.y + pos.height * p.percentage }
...
const getBestConnectionPoints = (
    ...
    if (forcedSourceSide) {
        ...
        if (pById) {
            ...
            const pBySide = sPorts.find(p => p.side === forcedSourceSide && p.id === forcedSourceSide) 
    ...
    if (forcedTargetSide) {
        ...
        if (pById) {
            ...
            const pBySide = tPorts.find(p => p.side === forcedTargetSide && p.id === forcedTargetSide)
    ...
    if (!forcedSourceSide || !forcedTargetSide) {
        if (forcedSourceSide && !forcedTargetSide) {
            ...
            let minDst = Infinity
            tPorts.forEach(tp => {
                const d = Math.abs(tp.x - (start?.x || 0)) + Math.abs(tp.y - (start?.y || 0))
                if (d < minDst) {
                    minDst = d
                    end = tp
                }
            ...
            let minDst = Infinity
            sPorts.forEach(sp => {
                const d = Math.abs(sp.x - (end?.x || 0)) + Math.abs(sp.y - (end?.y || 0))
                if (d < minDst) {
                    minDst = d
                    start = sp
                }
            ...
            if (Math.abs(dx) > Math.abs(dy)) {
                 if (dx > 0) { preferredSourceSide = 'right'; preferredTargetSide = 'left' }
                 else { preferredSourceSide = 'left'; preferredTargetSide = 'right' }
            } else {
                 if (dy > 0) { preferredSourceSide = 'bottom'; preferredTargetSide = 'top' }
                 else { preferredSourceSide = 'top'; preferredTargetSide = 'bottom' }
            }
    ...
    return { startX: start?.x || 0, startY: start?.y || 0, endX: end?.x || 0, endY: end?.y || 0, startSide: start?.side, endSide: end?.side }
}

const updateConnectedLines = (movedTaskId: string, currentX?: number, currentY?: number) => {
    ...
    lines.forEach(({ line, arrow, type, otherId }) => {
        ...
        if (sourceTask) {
            const dep = sourceTask.dependencies.find(d => (typeof d === 'string' ? d : d.taskId) === targetId)
        ...
        if (type === 'incoming') {
            ...
            const points = getBestConnectionPoints(otherPos, activePos, forcedSourceSide, forcedTargetSide, sourceTask, targetTask)
            ...
            const points = getBestConnectionPoints(activePos, otherPos, forcedSourceSide, forcedTargetSide, sourceTask, targetTask)
            startX = points.startX
            startY = points.startY
            endX = points.endX
            endY = points.endY
        ...
        if (depType === 'straight') {
             ...
             let depObj: any = null
             if (sourceTask) {
                 const d = sourceTask.dependencies.find(d => (typeof d === 'string' ? d : d.taskId) === targetId)
                 depObj = (d && typeof d === 'object') ? d : null
             }
             const cps = getCurveControlPoints(startX, startY, endX, endY, startSide, endSide, depObj?.controlPoints)
             if (cps.length > 0) {
                 const lastCP = cps[cps.length - 1]!
...
onMounted(() => {
  ...
  store.setExportImageHandler(async () => {
    ...
    try {
      ...
      const filename = `project-${store.projectInfo.code}-${new Date().toISOString().split('T')[0]}.png`
      ...
      console.log(`Exporting with ratio: ${exportRatio} (Zoom: ${currentZoom})`)
...
watch(() => store.selectedElement, drawChart, { deep: true })
watch(() => store.viewSettings.zoomLevel, (val) => {
  if (leaferApp) {
...
watch(() => store.viewSettings.resetViewTrigger, () => {
  if (leaferApp && leaferApp.tree) {
    leaferApp.tree.x = 0
    leaferApp.tree.y = 0
   }
 })
watch(() => store.viewSettings.fitViewTrigger, () => {
